...
name      = "Malcolm Roddy" -- TODO: Update your full name
studentID = 30573684    -- TODO: Update your student ID
...
test :: (Integral p, Integral b) => ([p] -> [b]) -> Int -> p -> (Bool, b)
...
pseudoRandoms :: (Integral a, Integral p) => p -> a -> [a]
pseudoRandoms k seed = tail $ iterate next seed
  where
    next 0 = seed
    next r = (r*r `div` 10^(k`div`2)) `mod` 10^k

checkSum :: (Foldable t, Integral c) => c -> t c -> c
